...
class Agent
{
    ...
    {
        if(Lib\Robot::register($ip,$port)){
            return Lib\Util::errCodeMsg(0,"注册成功");
        }
        return Lib\Util::errCodeMsg(1,"注册失败");
    }
    /**
     * worker回调中心服 任务执行状态
     * @param $tasks
    ...
    public static function getAgents($gets=[],$page=1,$pagesize=10)
    {
        ...
        foreach ($list as $k=>$task)
        {
            ...
            $tmp = Lib\Robot::$table->get($task["ip"].":".$task["port"]);
            if (!empty($tmp)){
            ...
            if (!empty($gids)){
                ...
                foreach ($gids as $gid){
                    if ($gid == "-1") {
                        ...
                        $gname[]= isset($groups[$gid["gid"]])?$groups[$gid["gid"]]:"未知";
                    }
                }
                $list[$k]["gname"] = $gname;
            }
    ...
    {
        if (empty($agent)) return Lib\Util::errCodeMsg(101,"参数为空");
        $gids = $agent["gids"];
        unset($agent["gids"]);
    ...
    {
        if (empty($id) || empty($agent)) return Lib\Util::errCodeMsg(101,"参数为空");
        $gids = $agent["gids"];
        unset($agent["gids"]);
    ...
    {
        if (empty($id)) return Lib\Util::errCodeMsg(101,"参数为空");
